...
def update_ticket_info_table(array):
    table = dynamodb.Table("LotteryAIBased")
    scores = []

    for dictionary in array:
        print(dictionary)
        GameName, GameNumber, Availability, PackSize, TotalTickets, URL, Availability_Rank, NetGain, NetGainRank, AdjustedOdds, AdjustedOddsRank, Price, TotalValuePrizesRank, TotalValuePrizes = None, None, None, None, None, None, None, None, None, None, None, None, None, None
        for key in list(dictionary.keys()):
            if key:
                if key == "Game Name":
                    GameName = dictionary[key]
                if key == "Game Number":
                    GameNumber = dictionary[key]
                if key == "Current Prize Availability":
                    Availability = dictionary[key]
                if key == "Ticket Price USD":
                    Price = dictionary[key]
                if key == "Adjusted Odds":
                    AdjustedOdds = dictionary[key]
                if key == "Pack Size":
                    PackSize = dictionary[key]
                if key == "Prize Pool (USD)":
                    TotalValuePrizes = dictionary[key]
                if key == "Tickets In Circulation":
                    TotalTickets = dictionary[key]
                if key == "Net Gain/Loss Per Ticket":
                    NetGain = dictionary[key]
                if key == "Urls":
                    URL = dictionary[key]
        # for value in [NetGain, NetGainRank, TotalValuePrizes, TotalValuePrizesRank, Availability, Availability_Rank, #AdjustedOdds, AdjustedOddsRank, Price]:
        if int(Price) == 75:
            Price = float(50)
        adj_odd = (float(AdjustedOdds) - .10) * 500
        print(adj_odd, "odds")
        av_ = 0.09 * (float(Availability) * float(Availability))
        print(av_, "")
        total_prizes = numpy.sqrt(numpy.sqrt(numpy.sqrt(numpy.sqrt(numpy.sqrt(numpy.sqrt(numpy.sqrt(float(TotalValuePrizes))))-2)))) * 1.09
        print(total_prizes, "total prizes")
        price = numpy.sqrt(numpy.sqrt(float(Price)))
        print(price, "price")
        pre_mod_score = (2 * numpy.sqrt(adj_odd + total_prizes - (3.42352*price*price*price))) * av_
        pre_mod_score = pre_mod_score / 1000
        pre_mod_score = ((pre_mod_score * 3) - ((1.23211*price*price*price*price) / 4)) + 50
        pre_mod_score = pre_mod_score * 1.06
        print(f"Pre Mod Score: {pre_mod_score}")
        scores.append(pre_mod_score)

        # TO DO FLOAT PRICE AND TOTAL TICKETS BOTH NEGATIVE PENALTIES
        response = table.update_item(
            Key={
                'Game Name': GameName,
                'Game Number': GameNumber
            },
            UpdateExpression="SET Price = :P, Score = :S, PackSize = :PS, TotalTickets = :TT, Availability = :AV, Odds = :AO, NetGain = :NG, TotalPrizes = :TVP, Urls = :URLs",
            ExpressionAttributeValues={
                ":TVP": decimal.Decimal(str(TotalValuePrizes)),
                ":AV": decimal.Decimal(str(Availability)),
                ":S": decimal.Decimal(str(pre_mod_score)),
                ":AO": decimal.Decimal(str(AdjustedOdds)),
                ":P": decimal.Decimal(str(Price)),
                ":PS": decimal.Decimal(str(PackSize)),
                ":TT": decimal.Decimal(str(TotalTickets)),
                ":NG": decimal.Decimal(str(NetGain)),
                ":URLs": URL
            },
            ReturnValues="UPDATED_NEW"
        )
    return 100/max(scores), "Coefficient -> New"
